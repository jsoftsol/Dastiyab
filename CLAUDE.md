...
**`CONTEXT.md` and `docs/AGENT_NOTES.md` load automatically** — a `SessionStart` hook (`.claude/settings.json` → `.claude/hooks/session-start-context.mjs`) injects both files' full contents into context at the start of every session, so you don't need to read them manually. Still skim `docs/PRD.md` yourself if the task touches product scope — it's intentionally not auto-loaded. `CONTEXT.md` contains the current phase status, what's been built, where we left off, and the immediate next step — this replaces reading individual source files to orient yourself. `docs/PRD.md` contains the product requirements and phase-by-phase feature scope. `docs/AGENT_NOTES.md` is a repo-local, git-tracked replacement for Claude Code's global auto-memory system for this project — durable feedback and project facts that don't fit the other docs. If the injected context is missing (e.g. right after the hook was first added, before a `/hooks` reload), fall back to reading both files manually.
